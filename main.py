...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get("SECRET_KEY", "bayonetta123")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            verify_user_result = check_password_hash(user.password, password_input)
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("Please login in or register to comment.")
            return redirect(url_for('login'))
        else:
            new_comment = Comment(
